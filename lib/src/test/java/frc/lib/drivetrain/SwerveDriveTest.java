...
class SwerveDriveTest {
  ...
  @BeforeEach
  void resetState() {
    drive.resetPose(new Pose2d(0, 0, Rotation2d.kZero));
  }
  ...
  void driveFieldCentricFacingPointReturnsCommand() {
    Command cmd =
        drive.driveFieldCentricFacingPoint(() -> 0, () -> 0, () -> new Translation2d(5, 5));
    assertNotNull(cmd);
    assertTrue(cmd.getRequirements().contains(drive));
  }
  ...
  void driveFieldCentricFacingPointExecutesWithoutError() {
    Command cmd =
        drive.driveFieldCentricFacingPoint(() -> 1.0, () -> 0.5, () -> new Translation2d(8, 4));
    assertDoesNotThrow(
        () -> {
          cmd.initialize();
          cmd.execute();
  ...
  void driveFieldCentricFacingPointCallsSupplierEachCycle() {
    int[] callCount = {0};
    Command cmd =
        drive.driveFieldCentricFacingPoint(
            () -> 0,
            () -> 0,
            () -> {
              callCount[0]++;
  ...
  @Test
  void distanceToReturnsCorrectDistance() {
    // Robot at origin (reset by @BeforeEach), point at (3, 4) → distance = 5
    assertEquals(5.0, drive.distanceTo(new Translation2d(3, 4)), 0.01);
  }

  @Test
  void distanceToUpdatesAfterResetPose() {
    drive.resetPose(new Pose2d(3, 0, Rotation2d.kZero));
    // Robot at (3,0), point at (6, 4) → distance = 5
    assertEquals(5.0, drive.distanceTo(new Translation2d(6, 4)), 0.05);
  }

  @Test
  void angleToReturnsCorrectAngle() {
    // Robot at origin, point ahead at (5, 0) → 0°
    assertEquals(0.0, drive.angleTo(new Translation2d(5, 0)).getDegrees(), 0.01);
    // Point to the left at (0, 5) → 90°
    assertEquals(90.0, drive.angleTo(new Translation2d(0, 5)).getDegrees(), 0.01);
  }

  @Test
  void isNearTrueWithinTolerance() {
    assertTrue(drive.isNear(new Translation2d(0.05, 0), 0.1));
  }

  @Test
  void isNearFalseOutsideTolerance() {
    assertFalse(drive.isNear(new Translation2d(1, 0), 0.5));
  }

  @Test
  void isAimedAtTrueWhenHeadingMatchesAngle() {
    // Robot at origin, heading 0° (default), point straight ahead at (5, 0)
    assertTrue(drive.isAimedAt(new Translation2d(5, 0), 5.0));
  }

  @Test
  void isAimedAtFalseWhenHeadingDoesNotMatch() {
    // Robot at origin, heading 0°, point to the left at (0, 5) → angle 90°
    assertFalse(drive.isAimedAt(new Translation2d(0, 5), 5.0));
  }

  @Test
  void getFieldRelativeVelocityZeroWhenStopped() {
    Translation2d vel = drive.getFieldRelativeVelocity();
    assertEquals(0.0, vel.getX(), 0.01);
    assertEquals(0.0, vel.getY(), 0.01);
  }
  ...
  private SwerveDrive createDriveWithIO(
  ...
  private SwerveDrive createDriveWithVisionIO(
    ...
    DrivetrainIO mockIO =
        inputs -> {
          ...
          inputs.visionPoseZ = new double[1];
  ...
  void visionSingleTagFarRejected() {
    ...
    Pose2d pose = d.getPose();
    assertEquals(
        0.0, pose.getX(), 0.01, "Single tag far should be rejected, pose stays at origin X");
    assertEquals(
        0.0, pose.getY(), 0.01, "Single tag far should be rejected, pose stays at origin Y");
  }
  ...
  void visionStdDevsReturnsNullForSingleTagFar() {
    assertNull(drive.getVisionStdDevs(1, 5.0), "Single tag >4m should return null (reject)");
  }

  @Test
  void visionStdDevsScalesForSingleTagClose() {
    var stdDevs = drive.getVisionStdDevs(1, 2.0);
    assertNotNull(stdDevs, "Single tag <4m should not be rejected");
    // linearStdDev = 0.02 * 4.0 / 1 = 0.08
    assertEquals(0.08, stdDevs.get(0, 0), 0.001);
    assertEquals(0.08, stdDevs.get(1, 0), 0.001);
    // angularStdDev = 0.06 * 4.0 / 1 = 0.24
    assertEquals(0.24, stdDevs.get(2, 0), 0.001);
  }

  @Test
  void visionStdDevsScalesForMultiTagClose() {
    var stdDevs = drive.getVisionStdDevs(2, 1.5);
    assertNotNull(stdDevs, "Multi-tag <4m should not be rejected");
    // linearStdDev = 0.02 * 2.25 / 2 = 0.0225
    assertEquals(0.0225, stdDevs.get(0, 0), 0.001);
    assertEquals(0.0225, stdDevs.get(1, 0), 0.001);
    // angularStdDev = 0.06 * 2.25 / 2 = 0.0675
    assertEquals(0.0675, stdDevs.get(2, 0), 0.001);
  }
  ...
  void isVisionPoseOnFieldReturnsFalseForOutOfBounds() {
    assertFalse(drive.isVisionPoseOnField(new Pose2d(50, 50, Rotation2d.kZero)));
    assertFalse(drive.isVisionPoseOnField(new Pose2d(-1, 4, Rotation2d.kZero)));
    assertFalse(drive.isVisionPoseOnField(new Pose2d(8, -1, Rotation2d.kZero)));
    assertFalse(drive.isVisionPoseOnField(new Pose2d(18, 4, Rotation2d.kZero)));
    assertFalse(drive.isVisionPoseOnField(new Pose2d(8, 9, Rotation2d.kZero)));
  }

  @Test
  void isVisionPoseOnFieldReturnsTrueForValidPose() {
    assertTrue(drive.isVisionPoseOnField(new Pose2d(8, 4, Rotation2d.kZero)));
    assertTrue(drive.isVisionPoseOnField(new Pose2d(0, 0, Rotation2d.kZero)));
    assertTrue(drive.isVisionPoseOnField(new Pose2d(17, 8.7, Rotation2d.kZero)));
    assertTrue(drive.isVisionPoseOnField(new Pose2d(16.5, 8.0, Rotation2d.kZero)));
  }

  @Test
  void visionZHeightRejected() {
    SwerveDrive d =
        createDriveWithVisionIO(
            inputs -> {
              inputs.visionConnected[0] = true;
              ...
              inputs.visionPoseY[0] = 2.0;
              inputs.visionPoseRotDeg[0] = 0;
              inputs.visionPoseZ[0] = 1.5; // well above threshold
              inputs.visionTimestampSec[0] = edu.wpi.first.wpilibj.Timer.getFPGATimestamp();
              inputs.visionAmbiguity[0] = 0.05;
              inputs.visionTagCount[0] = 2;
              inputs.visionAvgTagDistM[0] = 1.0;
    ...
    d.periodic();
    Pose2d pose = d.getPose();
    assertEquals(0.0, pose.getX(), 0.01, "Z-height violated vision pose should be rejected");
    assertEquals(0.0, pose.getY(), 0.01, "Z-height violated vision pose should be rejected");
  }
  ...
  void getPoseConfidenceReturnsDeadReckoningWithNoVision() {
    assertEquals(SwerveDrive.PoseConfidence.DEAD_RECKONING, drive.getPoseConfidence(10.0, 0, 1.0));
  }

  @Test
  void getPoseConfidenceReturnsHighForMultiTagRecent() {
    assertEquals(SwerveDrive.PoseConfidence.HIGH, drive.getPoseConfidence(0.5, 2, 0.05));
  }

  @Test
  void getPoseConfidenceReturnsMediumForOlderVision() {
    assertEquals(SwerveDrive.PoseConfidence.MEDIUM, drive.getPoseConfidence(1.5, 1, 0.15));
  }

  @Test
  void getPoseConfidenceReturnsLowForOldVision() {
    assertEquals(SwerveDrive.PoseConfidence.LOW, drive.getPoseConfidence(3.0, 1, 0.15));
  }
  ...
  void driveStateDefensiveCopyOnConstruction() {
    ...
    DriveState state =
        new DriveState(
            1.0, 25.0, 100.0, driveCurrents, steerCurrents, 12.5, false, 1.0, true, "OK", "none");
  ...
  @Test
  void characterizeWheelRadiusReturnsCommand() {
    Command cmd = drive.characterizeWheelRadius();
    assertNotNull(cmd);
    assertTrue(cmd.getRequirements().contains(drive));
  }
