...
class DrivetrainConfigTest {
  ...
  private static final PIDGains SIM_STEER_GAINS = new PIDGains(50, 0, 0.2, 0, 1.0, 0);
  private static final PIDGains SIM_DRIVE_GAINS = new PIDGains(0.05, 0, 0, 0, 0.1, 0);
  ...
  void rejectsInvalidCurrentLimits() {
    assertThrows(
        IllegalArgumentException.class, () -> validBuilder().currentLimits(0, 35, 20, 120));
    assertThrows(
        IllegalArgumentException.class, () -> validBuilder().currentLimits(40, 0, 20, 120));
    assertThrows(
        IllegalArgumentException.class, () -> validBuilder().currentLimits(40, 35, 0, 120));
  ...
  void buildsWithCameras() {
    ...
    DrivetrainConfig config = validBuilder().camera("front", t1).camera("back", t2).build();
  ...
  @Test
  void subConfigsDefaultWhenNotSet() {
    DrivetrainConfig config = validBuilder().build();
    assertNotNull(config.autoDrive);
    assertNotNull(config.visionConfig);
    assertNotNull(config.diagnosticsConfig);
    assertEquals(5.0, config.autoDrive.headingKP);
    assertEquals(0.2, config.visionConfig.maxAmbiguity);
    assertEquals(80.0, config.diagnosticsConfig.motorTempWarnC);
  }

  @Test
  void subConfigsAcceptCustomValues() {
    AutoDriveConfig auto = AutoDriveConfig.builder().headingKP(8.0).build();
    VisionConfig vision = VisionConfig.builder().maxAmbiguity(0.3).build();
    DiagnosticsConfig diag = DiagnosticsConfig.builder().motorTempWarn(70.0).build();
    DrivetrainConfig config =
        validBuilder().autoDrive(auto).visionConfig(vision).diagnosticsConfig(diag).build();
    assertSame(auto, config.autoDrive);
    assertSame(vision, config.visionConfig);
    assertSame(diag, config.diagnosticsConfig);
    assertEquals(8.0, config.autoDrive.headingKP);
    assertEquals(0.3, config.visionConfig.maxAmbiguity);
    assertEquals(70.0, config.diagnosticsConfig.motorTempWarnC);
  }

  // --- Sim PID gains ---

  @Test
  void simGainsDefaultToNull() {
    DrivetrainConfig config = validBuilder().build(false);
    assertNull(config.simSteerGains);
    assertNull(config.simDriveGains);
    assertSame(STEER_GAINS, config.activeSteerGains);
    assertSame(DRIVE_GAINS, config.activeDriveGains);
  }

  @Test
  void simGainsUsedWhenSimulation() {
    DrivetrainConfig config =
        validBuilder().simSteerPID(SIM_STEER_GAINS).simDrivePID(SIM_DRIVE_GAINS).build(true);
    assertSame(SIM_STEER_GAINS, config.activeSteerGains);
    assertSame(SIM_DRIVE_GAINS, config.activeDriveGains);
    assertSame(STEER_GAINS, config.steerGains);
    assertSame(DRIVE_GAINS, config.driveGains);
  }

  @Test
  void simGainsIgnoredWhenReal() {
    DrivetrainConfig config =
        validBuilder().simSteerPID(SIM_STEER_GAINS).simDrivePID(SIM_DRIVE_GAINS).build(false);
    assertSame(STEER_GAINS, config.activeSteerGains);
    assertSame(DRIVE_GAINS, config.activeDriveGains);
  }

  @Test
  void simGainsPartialOverride() {
    DrivetrainConfig config = validBuilder().simSteerPID(SIM_STEER_GAINS).build(true);
    assertSame(SIM_STEER_GAINS, config.activeSteerGains);
    assertSame(DRIVE_GAINS, config.activeDriveGains);
  }
  ...
  @Test
  void driveBaseRadiusComputesMaxDistance() {
    DrivetrainConfig config = validBuilder().build();
    // All modules at (±0.2921, ±0.2921), distance = sqrt(0.2921² + 0.2921²) ≈ 0.4131
    double expected = Math.hypot(0.2921, 0.2921);
    assertEquals(expected, config.driveBaseRadius(), 0.001);
  }
