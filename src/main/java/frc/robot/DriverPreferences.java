...
/**
 * Driver-tunable parameters stored via {@link Preferences} (persists across reboots). Values appear
 * in SmartDashboard under the Preferences widget and can be edited live.
 */
public final class DriverPreferences {
  ...
  public static double maxSpeedScale() {
    return MathUtil.clamp(
        Preferences.getDouble(MAX_SPEED_SCALE, DEFAULT_MAX_SPEED_SCALE), 0.1, 1.0);
  }

  /** Max rotational speed scale factor (0.0–1.0). */
  public static double maxRotationScale() {
    return MathUtil.clamp(
        Preferences.getDouble(MAX_ROTATION_SCALE, DEFAULT_MAX_ROTATION_SCALE), 0.1, 1.0);
  }
  ...
  public static double rotAccelLimit() {
    return MathUtil.clamp(
        Preferences.getDouble(ROT_ACCEL_LIMIT, DEFAULT_ROT_ACCEL_LIMIT), 1.0, 20.0);
  }
