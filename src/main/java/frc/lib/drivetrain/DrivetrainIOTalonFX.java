...
public class DrivetrainIOTalonFX implements DrivetrainIO {
    ...
    public DrivetrainIOTalonFX(
        ...
        for (int i = 0; i < 4; i++) {
            ...
            drivePositionSignals[i] = module.getDriveMotor().getPosition();
            driveTempSignals[i] = module.getDriveMotor().getDeviceTemp();
            steerTempSignals[i] = module.getSteerMotor().getDeviceTemp();
            driveCurrentSignals[i] = module.getDriveMotor().getStatorCurrent();
            steerCurrentSignals[i] = module.getSteerMotor().getStatorCurrent();
        }
    ...
    public void updateInputs(DrivetrainIOInputsAutoLogged inputs) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            inputs.drivePositionRad[i] = drivePositionSignals[i].getValueAsDouble()
